...
public class MainWindow extends javax.swing.JFrame {
    ...
    public MainWindow() throws SQLException, IOException {
        ...
        new SwingWorker<OperationsList, Void>() {
            ...
            protected OperationsList doInBackground() throws SQLException {
                try {
                    Date min = java.sql.Date.valueOf(LocalDate.of(1900, 01, 01));
                    Date max = java.sql.Date.valueOf(LocalDate.of(2200, 01, 01));
        ...
        opsTable.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent mouseEvent) {
                ...
                if (mouseEvent.getClickCount() == 2 && column == 0 && opsTable.getModel() instanceof CurrentPlanTableModel) {
                    giveOrderSeachSwingWorker(((CurrentPlanTableModel) opsTable.getModel()).getOpForRow(opsTable.rowAtPoint(point)).getOrderNo(), mouseEvent).execute();
                }
            }
        });
        ...
        ordersComboBox.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent e) {
                ...
                if (ordersComboBox.getSelectedItem() == null) {
                    return;
                }
                giveOrderSeachSwingWorker(((OrdersComboBoxItem) ordersComboBox.getSelectedItem()).getOrderNo(), e).execute();
            }
        });
    ...
    private void initComponents() {
        ...
        ordersComboBox = new javax.swing.JComboBox<OrdersComboBoxItem>();
        ...
        for(int i = 2; i < 13; i++) {
            insertIntoCombo(noteCategoryComboBox, new thesis.planningui.NoteCategoryClass(i, NoteCategoryClass.getDescWithCat(i)));
        }
    ...
    private SwingWorker giveOrderSeachSwingWorker(String itemNo, AWTEvent evt) {
            ...
            protected OperationsList doInBackground() throws SQLException {
                try {
                    for (int i = 0; i < ordersComboBox.getItemCount(); i++) {
                        if (ordersComboBox.getItemAt(i).getItemNo().equalsIgnoreCase(itemNo)) {
                            return OperationsList.showOrder(ordersComboBox.getItemAt(i).getOrderNo());
                        }
                    }
                    return OperationsList.showOrder(itemNo);
                } catch (SQLException ex) {
            ...
            protected OperationsList doInBackground() throws SQLException {
                try {
                    ...
                    if (allTimeCheckBox.isSelected()) {
                        dates = new DatePair(min, max);
                    } else {
                        dates = checkDates(fromDatePicker.getDate(), toDatePicker.getDate());
                        if (dates == null) {
                            this.cancel(true);
                        }
                    }
    ...
    private void addNewNoteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addNewNoteButtonActionPerformed
        ...
        if (JOptionPane.showOptionDialog(null, delayInfoPanel, "Zadaj informácie o poznámke", 0, JOptionPane.INFORMATION_MESSAGE, null, options, null) == 0) {
            ...
            CurrentPlanTableModel tableModel = (CurrentPlanTableModel) opsTable.getModel();
            loadingLabel.setVisible(true);
    ...
    private void showMatrixButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showMatrixButtonActionPerformed
        ...
        new SwingWorker<OverviewMatrixTableModel, Void>() {
            ...
            protected void done() {
                try {
                    OverviewMatrixTableModel model = this.get();
    ...
    private void showRescheduledButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showRescheduledButtonActionPerformed
        ...
        new SwingWorker<OperationsList, Void>() {
            ...
            protected OperationsList doInBackground() throws SQLException {
                try {
                    DatePair dates;
                    if (allTimeCheckBoxR.isSelected()) {
                        dates = new DatePair(min, max);
                    } else {
                        dates = checkDates(fromDatePickerR.getDate(), toDatePickerR.getDate());
                        if (dates == null) {
                            this.cancel(true);
                        }
                    }
    ...
    private void fillTableWithOperations(OperationsList opL, AWTEvent evt) {
        CurrentPlanTableModel model = new CurrentPlanTableModel(opL);
        opsTable.setModel(model);
        model.resize(opsTable);
        if (Arrays.asList(searchWithOrderNoButton, ordersComboBox, opsTable, orderNoTextField).contains(evt.getSource())) {
    ...
    private DatePair checkDates(Date from, Date to) {

        if (from.before(min) || from.after(max) || to.before(min) || to.after(max)
                || to.before(from)) {
            JOptionPane.showMessageDialog(null, "Zle zadaný dátum.", "Upozornenie", JOptionPane.WARNING_MESSAGE);
            return null;
        }
        return new DatePair(from, to);
    }
